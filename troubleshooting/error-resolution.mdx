...
_`(Typically from /oauth/token endpoint)`_
...
_```(Typically from /oauth/token endpoint when using authorization_code or refresh_token grant types)```_
...
_```(From /oauth/authorize or /oauth/token if scope validation occurs, or /api/* endpoints if token lacks required scope for a resource)```_
...
_`(Typically from /oauth/authorize redirect)`_
...
_`(Typically from /api/user/details or other sensitive API endpoints)`_
