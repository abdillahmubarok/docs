...
class MubarokahController extends Controller
{
    ...
    public function handleProviderCallback(Request $request)
    {
        ...
        try {
            ...
            // Best Practice: Use mubarokah_id as the primary lookup key
            // because 'email' can be null for WhatsApp-registered users.
            $user = User::where('mubarokah_id', $mubarokahUser['id'])->first();

            if (!$user && isset($mubarokahUser['email'])) {
                // Fallback lookup by email if mubarokah_id is not yet set
                $user = User::where('email', $mubarokahUser['email'])->first();
            }

            if (!$user) {
                $user = new User();
                $user->mubarokah_id = $mubarokahUser['id'];
            }

            // Sync user data
            $user->name = $mubarokahUser['name'];
            $user->email = $mubarokahUser['email'] ?? ($mubarokahUser['id'] . '@mubarokah.id'); // Fallback email
            $user->username = $mubarokahUser['username'] ?? $user->email;
            $user->avatar = $mubarokahUser['profile_picture'] ?? null;
            $user->save();
