...
If using HTTP Basic Auth, include a header like: `Authorization: Basic base64(your_client_id:your_client_secret)`.
...
<Tabs>
  <Tab title="Authorization Code Grant">
    ...
    **Request Body Parameters:**
    ...
    <ParamField body="code" type="string" required={true}>
      The authorization code received from the `/oauth/authorize` redirect.
    </ParamField>
    <ParamField body="redirect_uri" type="string" required={true}>
      The same `redirect_uri` that was used in the initial authorization request.
    </ParamField>
    <ParamField body="client_id" type="string" required={true}>
      Your application's Client ID. (Required if not using HTTP Basic Auth header for client authentication).
    </ParamField>
    <ParamField body="client_secret" type="string" required={true}>
      Your application's Client Secret. (Required if not using HTTP Basic Auth header for client authentication).
    </ParamField>
    <ParamField body="code_verifier" type="string">
      If PKCE was used in the authorization request, this is the original `code_verifier` string. Required for public clients that used PKCE.
    </ParamField>
    ...
    **Example Request (Node.js - Server-Side):**
    ...
    async function exchangeCodeForTokens(code, codeVerifier) {
      ...
      // If not using Basic Auth header, include client_id and client_secret in body
  ...
  <Tab title="Refresh Token Grant">
    ...
    **Request Body Parameters:**
    ...
    <ParamField body="refresh_token" type="string" required={true}>
      The refresh token issued to your application.
    </ParamField>
    <ParamField body="client_id" type="string" required={true}>
      Your application's Client ID. (Required if not using HTTP Basic Auth).
    </ParamField>
    <ParamField body="client_secret" type="string" required={true}>
      Your application's Client Secret. (Required if not using HTTP Basic Auth).
    </ParamField>
    <ParamField body="scope" type="string">
      Optional. A space-separated list of scopes. You can request a subset of the original scopes. If omitted, the new access token will have the same scopes as the original, or a default set if the refresh token is not scoped.
    </ParamField>
  ...
  <Tab title="Client Credentials Grant">
    ...
    **Request Body Parameters:**
    ...
    <ParamField body="scope" type="string">
      Optional. A space-separated list of scopes relevant to your application's own operations (e.g., `internal-stats-access`).
    </ParamField>
...
<ResponseField name="access_token" type="string" required={true}>
  The access token that your application can use to make requests to protected resources on behalf of the user or itself.
</ResponseField>
<ResponseField name="token_type" type="string" required={true}>
  Typically `Bearer`. Indicates how the access token should be used in API requests (e.g., `Authorization: Bearer <access_token>`).
</ResponseField>
<ResponseField name="expires_in" type="number" required={true}>
  The lifetime of the access token in seconds (e.g., `3600` for 1 hour, `86400` for 24 hours).
</ResponseField>
<ResponseField name="refresh_token" type="string">
  (Optional) An opaque token that can be used to obtain a new access token when the current one expires. Usually returned with the `authorization_code` grant.
</ResponseField>
<ResponseField name="scope" type="string">
  (Optional) If the granted scopes are different from what was requested, this parameter will list the actual granted scopes.
</ResponseField>
